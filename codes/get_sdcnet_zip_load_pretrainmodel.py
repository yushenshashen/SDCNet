...
"""
Usage:
nohup python get_sdcnet_zip_load_pretrainmodel.py  >> logs/log_sdcnet_zip_load_pretrainmodel.txt 2>&1 &
...
flags.DEFINE_string('modelfile', '../trained_model/sdcnet_zip/best_model.ckpt', 'the path of the trained model file')
...
import numpy as np
import pandas as pd
data = pd.read_csv('../data/oneil_dataset_zip.txt', sep='\t', header=0)
data.columns = ['drugname1','drugname2','cell_line','synergy']
# data.columns = ['drugname1','drugname2','cell_line', 'zip', 'zip', 'loewe', 'hsa']
...
resultspath = '../results_oneil/results_sdcnet_zip3'
...
for cellidx in range(cellscount):
    ...
    net1_data = each_data[each_data['synergy'] >= 2.64]
    net2_data = each_data[each_data['synergy'] < -4.48] 
    net3_data = each_data[(each_data['synergy'] >= -4.48) & (each_data['synergy'] < 2.64)]
