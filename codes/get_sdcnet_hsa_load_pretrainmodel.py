...
"""
Usage:
nohup python get_sdcnet_hsa_load_pretrainmodel.py  >> logs/log_sdcnet_hsa_load_pretrainmodel.txt 2>&1 &
...
flags.DEFINE_string('modelfile', '../trained_model/sdcnet_hsa/best_model.ckpt', 'the path of the trained model file')
...
import numpy as np
import pandas as pd
data = pd.read_csv('../data/oneil_dataset_hsa.txt', sep='\t', header=0)
data.columns = ['drugname1','drugname2','cell_line','synergy']
# data.columns = ['drugname1','drugname2','cell_line', 'zip', 'hsa', 'loewe', 'hsa']
...
resultspath = '../results_oneil/results_sdcnet_hsa3'
...
for cellidx in range(cellscount):
    ...
    net1_data = each_data[each_data['synergy'] >= 3.87]
    net2_data = each_data[each_data['synergy'] < -3.02] 
    net3_data = each_data[(each_data['synergy'] >= -3.02) & (each_data['synergy'] < 3.87)]
